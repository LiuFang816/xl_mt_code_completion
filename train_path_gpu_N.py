...
def get_model_fn(n_token_N, n_token_T, cutoffs, alpha):
    def model_fn(inpN, inpT, tgtN, inpPath, h_par, mems, is_training):
...
def single_core_graph(n_token_N, n_token_T, cutoffs, is_training, inpN, inpT, tgtN, inputPath, h_par, mems,
                      alpha):
...
def train(train_data, valid_data, n_token_N, n_token_T, cutoffs, vocab_size, fout):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            print('this run_epoch takes time %.2f' % (time.time() - start_time), file=fout)
            fout.flush()
            # evaluate
...
def main(unused_argv):
    outfile = 'logs/JS_output_mt_path_N.txt'
    fout = open(outfile, 'a')

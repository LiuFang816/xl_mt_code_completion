...
def get_model_fn(n_token_N, n_token_T, cutoffs, alpha):
    def model_fn(inpN, inpT, tgtT, mems, is_training):
        ...
        lossT, new_mems, predictionT = model.transformer(
            inpN=inpN,
            inpT=inpT,
            targetsT=tgtT,
            mems=mems,
        ...
        if is_training:
            ...
            return loss, new_mems, grads_and_vars, predictionT
        else:
            return loss, new_mems, predictionT

    return model_fn


def single_core_graph(n_token_N, n_token_T, cutoffs, is_training, inpN, inpT, tgtT, mems, alpha):
...
def train(train_data, valid_data, n_token_N, n_token_T, cutoffs, vocab_size, fout):
    ...
    targetsT = tf.placeholder(dtype=tf.int32, shape=[FLAGS.train_batch_size, FLAGS.tgt_len], name="targetsN")
    ...
    tower_mems, tower_losses, tower_new_mems, tower_grads_and_vars, predictionT = [], [], [], [], []
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            fetches = [loss, tower_new_mems, global_step, gnorm, learning_rate, train_op, acc_T]
            ...
            while True:
                ...
                if curr_step > 0 and curr_step % FLAGS.iterations == 0:
                    ...
                    tf.logging.info("Epoch {}  |  [{}] | gnorm {:.2f} lr {:8.6f} "
                                    "| loss {:.2f} | pplx {:>7.2f}, accT {:.4f} bpc {:>7.4f}".format(
                        epoch + 1, curr_step, fetched[-4], fetched[-3],
                        curr_loss, math.exp(curr_loss), accuracy_T, curr_loss / math.log(2)))
                    total_loss, prev_step = 0., curr_step
                    print("Epoch {}  |  [{}] | gnorm {:.2f} lr {:8.6f} "
                          "| loss {:.2f} | pplx {:>7.2f}, accT {:.4f} bpc {:>7.4f}".format(
                        epoch + 1, curr_step, fetched[-4], fetched[-3],
                        curr_loss, math.exp(curr_loss), accuracy_T, curr_loss / math.log(2)), file=fout)
                    fout.flush()
            ...
            tf.logging.info(
                "Validation:  Epoch {}  | loss {:.2f} | pplx {:>7.2f}, accT {:.4f}, bpc {:>7.4f}".format(
                    epoch + 1, total_loss / epoch_size, math.exp(total_loss / epoch_size), total_accT / epoch_size,
                    (total_loss / epoch_size) / math.log(2)))
            print("Validation:  Epoch {}  | loss {:.2f} | pplx {:>7.2f}, accT {:.4f}, bpc {:>7.4f}".format(
                epoch + 1, total_loss / epoch_size, math.exp(total_loss / epoch_size), total_accT / epoch_size,

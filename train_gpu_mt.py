...
N_filename = '/data/liufang/mt/JS_data/pickle_data/JS_non_terminal.pickle'
T_filename = '/data/liufang/mt/JS_data/pickle_data/JS_terminal_50k_whole.pickle'
...
flags.DEFINE_integer("epochs", default=30,
                     help="epoch size.")
flags.DEFINE_integer("train_batch_size", default=90,
...
# Model config
flags.DEFINE_integer("tgt_len", default=50,
...
def single_core_graph(n_token_N, n_token_T, cutoffs, is_training, inpN, inpT, tgtN, tgtT, mems, alpha):
...
def train(train_data, valid_data, n_token_N, n_token_T, cutoffs, vocab_size, fout, wf=None):
    ...
    # self.targetsS = tf.placeholder(dtype=tf.int32, shape=[batch_size, num_steps], name="targetsS")
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            # evaluate
            eval_fetches = [loss, tower_new_mems, acc_N, acc_T, predictionN]
            total_loss, prev_step = 0., -1
            total_accN = 0.
            total_accT = 0.
            ...
            while True:
                ...
                accuracy_N, accuracy_T = (eval_fetched[-3], eval_fetched[-2])

                pred = eval_fetched[-1]
                targets = np.reshape(tN, [-1])

                if wf is not None:
                    wf.write(json.dumps(targets.tolist()) + '\n')
                    wf.write(json.dumps(pred.tolist()) + '\n')
...
def main(unused_argv):
    outfile = 'logs/JS_output_mt_{}.txt'.format(FLAGS.alpha)
    fout = open(outfile, 'a')
